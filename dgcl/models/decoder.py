...
class dec_deeplabv3_plus(nn.Module):
    ...
    def forward(self, x):
        ...
        fts = self.classifier(aspp_out)
        res = {'fts':fts, "pred":self.final(fts.clone())} # "pred":self.final(fts.clone())

        if self.rep_head:
            # b,c,h,w = aspp_out.size()
            # rep_out = self.representation(aspp_out.permute(0,2,3,1).contiguous().view(b*h*w, c))
            b,c,h,w = fts.size()
            rep_out = self.representation(fts.permute(0,2,3,1).contiguous().view(b*h*w, c))

            res["rep"] = rep_out.view(b,h,w,256).permute(0,3,1,2) 
