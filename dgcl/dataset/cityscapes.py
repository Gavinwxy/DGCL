...
class city_dset(BaseDataset):
    def __init__(self, data_root, data_list, trs_form, seed, n_sup, split="val"):
        ...
        if self.split != "val":
            self.crop_unsup = psp_trsform.Crop([769,769], crop_type="rand", ignore_label=254)
            self.crop = psp_trsform.Crop([769,769], crop_type="rand", ignore_label=255)
...
def build_city_semi_loader(split, all_cfg, seed=0):
    ...
    dset = city_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup, 'sup')

    
    # build sampler for unlabeled set
    data_list_unsup = cfg["data_list"].replace("labeled.txt", "unlabeled.txt")
    dset_unsup = city_dset(cfg["data_root"], data_list_unsup, trs_form_unsup, seed, n_sup, 'unsup')
